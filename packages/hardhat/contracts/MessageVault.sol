...
contract MessageVault {
     ...
     * @param message The single character to display in the cell (will be uppercased)
     */
    function buyCell(uint256 cellId, string memory message) external payable {
        require(cellId < TOTAL_CELLS, "Invalid cell ID");
        require(bytes(message).length > 0, "Character cannot be empty");
        require(bytes(message).length <= 1, "Only 1 character allowed per cell");
    ...
    function updateMessage(uint256 cellId, string memory newMessage) external {
        require(cellId < TOTAL_CELLS, "Invalid cell ID");
        require(bytes(newMessage).length > 0, "Character cannot be empty");
        require(bytes(newMessage).length <= 1, "Only 1 character allowed per cell");
